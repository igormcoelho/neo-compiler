        ...
        public bool IsInlineCall(Mono.Cecil.MethodDefinition defs, out VM.OpCode[] opcodes, out string[] names, out bool[] isHex)
        {
            ...
            foreach (var attr in defs.CustomAttributes)
            {
                if (attr.AttributeType.Name == "InlineAttribute")
                {
                    ...
                    VM.Opcode val1 = attr.ConstructorArguments[0].Value;
                    string val2 = attr.ConstructorArguments[1].Value;
                    bool val3 = attr.ConstructorArguments[2].Value;
        ...
        private int _ConvertCall(OpCode src, NeoMethod to)
        {
            ...
            {//maybe a syscall // or other
                ...
                else if (src.tokenMethod.Contains("ByteArrayX::set_Item(System.Int32,System"))
                {
                  ...
                  logger.Log($"log Y prev3 line {to.body_Codes[addr-3].ToString()} is interval {((to.body_Codes[addr-2].code >= VM.OpCode.PUSH0) && (to.body_Codes[addr-3].code <= VM.OpCode.PUSH16)).ToString()}");
                ...
                else if (src.tokenMethod.Contains("ByteArrayX::get_Item(System.Int32"))
                {
                    ...
                    logger.Log($"log prev1 line {to.body_Codes[addr-1].ToString()}");
                    logger.Log($"log prev2 line {to.body_Codes[addr-2].ToString()}");
